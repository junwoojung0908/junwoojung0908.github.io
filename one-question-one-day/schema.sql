...
--    to "Redirect URLs" (e.g. https://username.github.io/one-question-one-day/index.html)
-- ============================================================
...
-- Answers: publicly readable (feed visibility is enforced client-side)
CREATE POLICY "answers_select_public" ON answers
  FOR SELECT USING (true);

-- Answers: authenticated users may insert their own row only
CREATE POLICY "answers_insert_own" ON answers
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id);

-- ── Sample questions (edit as needed) ────────────────────────
