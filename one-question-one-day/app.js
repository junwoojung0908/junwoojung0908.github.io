...
async function submitAnswer(content) {
  ...
  // 23505 = unique violation (already answered today) — treat as success
...
async function init() {
  // getSession() in Supabase v2 auto-processes magic link token from URL hash
  ...
  if (currentUser) {
    // Check for pending answer saved before magic link auth
...
elSubmitBtn.addEventListener('click', async () => {
  ...
  if (!currentUser) {
    // Save draft and request magic link auth
...
elMagicLinkBtn.addEventListener('click', async () => {
  ...
  // Redirect back to this exact page after auth
  ...
  elAuthMsg.classList.remove('error');
  elAuthMsg.textContent = '인증 메일을 보냈습니다. 메일함을 확인해 링크를 클릭하세요.';
...
elBackBtn.addEventListener('click', () => {
  ...
  if (raw) {
    try {
      const pending = JSON.parse(raw);
      if (pending.content) {
        elAnswerInput.value = pending.content;
        elCharCount.textContent = pending.content.length;
      }
    } catch (_) { /* ignore */ }
  }
});
