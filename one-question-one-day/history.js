...
function renderHistory(answers) {
  ...
  const html = sorted.map(([date, rows]) => {
  ...
  if (!user) {
    ...
    elContent.innerHTML = `
      <p class="history-empty">
        로그인이 필요합니다.
        <a href="index.html" style="color:inherit;text-decoration:underline;text-underline-offset:3px">오늘의 질문으로</a>
      </p>`;
    return;
  }
